...
This builds the module, starts an Envoy instance with the filter configured, and runs tests against it using [func-e](https://func-e.io/) to manage the Envoy binary.

### Docker

```bash
docker build -t envoy-latency-fault .
docker run -v $(pwd)/integration/envoy.yaml:/etc/envoy/envoy.yaml envoy-latency-fault -c /etc/envoy/envoy.yaml
```

## Module Discovery

Set `ENVOY_DYNAMIC_MODULES_SEARCH_PATH` to the directory containing `liblatency_fault_module.so`. Envoy will look for `${ENVOY_DYNAMIC_MODULES_SEARCH_PATH}/liblatency_fault_module.so`.
